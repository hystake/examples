...
r = requests.get('http://dev.hystake.com/api/v1/dataset/' + dataset_code + '?token=' + token + query)

# example 1: return the last 3 day's crude oil prices
r1 = requests.get('http://dev.hystake.com/api/v1/dataset/HYSTAKE-CMEFUT-CL1/?token=' + token + '&sort=date.desc&limit=3')

# example 2: return the gold futures price since May 1, 2020
r2 = requests.get('http://dev.hystake.com/api/v1/dataset/HYSTAKE-CMEFUT-GC1/?token=' + token + '&filter=date__gt=05-01-2020')

# example 3: return the latest closing price of Eurodollar futures
r3 = requests.get('http://dev.hystake.com/api/v1/dataset/HYSTAKE-CMEFUT-ED1/?token=' + token + '&field=close&last=true')
